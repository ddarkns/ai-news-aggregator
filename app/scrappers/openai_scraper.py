"""Simple OpenAI News RSS scraper."""
...
class NewsArticle(BaseModel):
    ...
    @classmethod
    def from_entry(cls, entry: feedparser.FeedParserDict) -> "NewsArticle":
...
class OpenAINewsScraper:
    """Fetches articles from OpenAI's News RSS feed."""

    def __init__(self, rss_url: str = RSS_URL, headless: bool = False) -> None:
        self.rss_url = rss_url
        self.headless = headless

    def fetch_articles(self, limit: Optional[int] = None) -> List[NewsArticle]:
        feed = feedparser.parse(self.rss_url)
        entries = feed.entries[:limit] if limit else feed.entries
        return [NewsArticle.from_entry(entry) for entry in entries]
    ...
    def batch_to_markdown(self, articles: List[NewsArticle], max_articles: Optional[int] = None) -> List[str]:
        """
...
if __name__ == "__main__":
    scraper = OpenAINewsScraper(headless=False)  # Set headless=False to see browser
    ...
    if articles:
        ...
        print(f"Converting: {articles[0].title}")
